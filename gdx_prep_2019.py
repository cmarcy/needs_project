...
os.chdir('C:/Users/cmarcy/Desktop/py_projects/plantfile/NEEDS_project')
...
CONVOLDqctn = CONVOLDqctn[['tech','coolingwatertech','pca','cap']].groupby(by=['tech','coolingwatertech','pca'], as_index=False).sum().reset_index(drop=True)
...
tmpCSPOct = tmpCSPOct[['resource_region','cap']].groupby(by=['resource_region'], as_index=False).sum()
...
if not os.path.exists(os.path.join('post_ct_mapping','post_wst_implementation')):
    os.mkdir(os.path.join('C:/Users/cmarcy/Desktop/py_projects/plantfile','post_ct_mapping'))
    os.mkdir(os.path.join('C:/Users/cmarcy/Desktop/py_projects/plantfile','post_ct_mapping','post_wst_implementation'))
...
leading_cols = ["tech","pca","resource_region","cap","Nuke60RetireYear","Nuke80RetireYear","NukeEarlyRetireYear","NukeRefRetireYear","RetireYear","Commercial.Online.Year.Quarter","IsExistUnit"]
...
nems.to_csv('needs_final3.csv',index=False)
